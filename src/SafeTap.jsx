  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 10, height: 10, overflow: "hidden" }}>
        <div style={{
          height: "100%", borderRadius: 10, transition: "width 1s ease", width: `${score}%`,
...
export default function SafeTap() {
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      const res = await API.loginPin(fullPhone, pin);
      ...
      if (res.authToken) {
        ...
        try {
          ...
          if (Array.isArray(gList)) setGuardians(gList);
        } catch (ge) {
          console.warn("Could not load guardians:", ge);
        }
        setLoggedIn(true);
      } else {
        console.error("Login failed:", res);
        setAuthErr(`⚠️  ${res.message || "Wrong credentials — check phone & PIN"}`);
      }
    } catch (e) {
  ...
  const doRegister = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        age: reg.age,
        dob: reg.dob,
        blood_group: reg.blood,
        address: reg.address,
        city: reg.city,
        state: reg.state,
        pincode: reg.pincode,
        bank_name: reg.bank,
        account_last4: reg.acc,
        ifsc: reg.ifsc,
      };
      ...
      if (res.authToken) {
        localStorage.setItem("st_token", res.authToken);
        const userData = { ...reg, phone: fullPhone, email, ...(res.user || {}) };
        setProfile(p => ({ ...p, ...userData }));
        setLoggedIn(true);
      } else {
        console.error("Registration failed:", res);
        setAuthErr(`⚠️  ${res.message || "Registration failed — check all fields"}`);
      }
    } catch (e) {
  ...
  const saveProfile = async () => {
    try {
      ...
      setProfile({ ...draft }); // fallback to local
    }
    setEditMode(false);
  ...
  const addGuardian = async () => {
    ...
    try {
      ...
      console.log("Guardian saved:", saved);
      if (saved && (saved.id || saved._id)) {
        setGuardians(g => [...g, saved]);
      } else {
        // API call succeeded but returned unexpected shape; use local fallback
        console.warn("Guardian API response unexpected:", saved);
        setGuardians(g => [...g, { ...newG, id: Date.now() }]);
      }
    } catch (e) {
      console.error("Add guardian error:", e);
      // Still add locally so user sees it (will be lost on refresh if API failed)
      setGuardians(g => [...g, { ...newG, id: Date.now() }]);
      notify("⚠️  Saved locally — check network", "warn");
  ...
  if (!loggedIn) {
    return (
      <div style={{ fontFamily: F, background: BG, minHeight: "100vh", maxWidth: 480, margin: "0 auto", overflowX: "hidden", position: "relative" }}>
        ...
        {/* toast */}
        <div style={{
          position: "fixed", top: 18, left: "50%", transform: `translateX(-50%) translateY(${toast.show ? 0 : -90}px)`, zIndex: 999,
        ...
        <div style={{ position: "relative", zIndex: 1, paddingBottom: 52 }}>
          ...
          <div style={{
            margin: "0 18px", ...glass(), padding: "28px 22px 24px",
  ...
  /* ── SCREEN: GUARDIANS (rendered inline to avoid re-mount on keystrokes) ── */
  const guardiansScreenJSX = (
    <div style={{ padding: "0 0 108px" }}>
      <div style={cardWrap()}>
        <div style={{ ...cHead, justifyContent: "space-between" }}>
        ...
        <div style={{ padding: "14px 20px" }}>
          ...
          {guardians.map((g, i) => (
            <div key={g.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 0", borderBottom: i < guardians.length - 1 ? "1px solid rgba(255,255,255,.06)" : "none" }}>
              ...
              }} style={{ background: "rgba(239,68,68,.12)", border: "1px solid rgba(239,68,68,.3)", borderRadius: 10, padding: "7px 12px", color: "#fca5a5", fontSize: 12, cursor: "pointer", fontFamily: F, fontWeight: 700 }}>Remove</button>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setAddGOpen(false)}>
          <div style={{ width: "100%", maxWidth: 480, ...glass({ borderRadius: "24px 24px 0 0", border: "1px solid rgba(29,78,216,.25)" }), padding: "26px 22px 44px", animation: "st_pop .3s ease" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 14 }}>
              <span style={lbl}>{t(lang, "guardianName")}</span>
              <input value={newG.name} onChange={e => setNewG(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Priya Kumar" type="text" style={inputSm()} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <span style={lbl}>{t(lang, "guardianPhone")}</span>
              <input value={newG.phone} onChange={e => setNewG(p => ({ ...p, phone: e.target.value }))} placeholder="+91-98765-43210" type="tel" style={inputSm()} />
            </div>
          ...
          {COMPLAINT_STEPS.map((step, idx) => {
            ...
            return (
              <div key={step.id} style={{ display: "flex", gap: 12, marginBottom: 14, alignItems: "flex-start" }}>
                <div style={{
                  width: 30, height: 30, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800,
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ background: "rgba(255,255,255,.09)", borderRadius: 10, height: 4, overflow: "hidden" }}>
                    <div style={{
                      height: "100%", borderRadius: 10, width: `${done ? 100 : active ? 50 : 0}%`,
  ...
  const SCREENS = { home: HomeScreen, progress: ProgressScreen, profile: ProfileScreen, info: InfoScreen };
  ...
  return (
    <div style={{ fontFamily: F, background: BG, minHeight: "100vh", maxWidth: 480, margin: "0 auto", overflowX: "hidden", position: "relative" }}>
      ...
      {screen === "guardians" ? guardiansScreenJSX : <Active />}
