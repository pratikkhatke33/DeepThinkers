...
const COMPLAINT_STEPS = [
  { id:1, label:"Scam Detected & AI Logged",       pct:10  },
  { id:2, label:"Emergency Alert → Guardians",     pct:20  },
  { id:3, label:"Transaction Frozen by SafeTap",   pct:35  },
  { id:4, label:"Complaint Filed – Cyber Cell 1930",pct:55 },
  { id:5, label:"FIR Filed – Nearest Police Station",pct:70},
  { id:6, label:"Bank Notified & Evidence Secured", pct:80 },
  { id:7, label:"Case Under Active Investigation",  pct:90 },
  { id:8, label:"Case Resolved & Closed",           pct:100 },
];
const COUNTRY_CODES = [
  ...
  return (
    <div style={{marginBottom:12}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
        <span style={{fontSize:12,color:"#90a4ae"}}>AI Scam Risk Score</span>
...
export default function SafeTap() {
  ...
  const doLogin = () => {
    ...
    if (locked)                   { setAuthErr("⚠️  Account locked");              return; }
    if (pin==="1234") { setLoggedIn(true); }
    else {
      const left = tries-1; setTries(left);
      if (left<=0) { setLocked(true); setAuthErr("⚠️  Account locked — too many wrong attempts"); }
      else setAuthErr(`⚠️  Wrong credentials — ${left} attempt(s) left`);
    }
  };
  ...
  const doFreeze = () => {
    if (frozen) return;
    setFrozen(true); setGAlert(true);
    notify(`🔒  Transaction FROZEN — Guardians alerted!`,"ok",4000);
    setTimeout(()=>setProgress(55),1600);
    setTimeout(()=>setProgress(70),3600);
  };
  const saveProfile = () => { setProfile({...draft}); setEditMode(false); notify("✅  Profile updated successfully!"); };
  const addGuardian = () => {
    if (!newG.name||!newG.phone) { notify("Fill in all fields","err"); return; }
    setGuardians(g=>[...g,{...newG,id:Date.now()}]);
    setNewG({name:"",phone:"",relationship:"Son"}); setAddGOpen(false);
      ...
      <div style={{fontFamily:F, background:BG, minHeight:"100vh", maxWidth:480, margin:"0 auto", overflowX:"hidden", position:"relative"}}>
        ...
        <div style={{position:"relative",zIndex:1,paddingBottom:52}}>
          ...
          <div style={{margin:"0 18px",...glass(),padding:"28px 22px 24px",
            ...
            <h2 style={{margin:"0 0 4px",fontSize:26,fontWeight:900,color:"#f8fafc",fontFamily:FD}}>Welcome Back</h2>
            <p  style={{margin:"0 0 22px",fontSize:13,color:"#64748b"}}>Your trusted protection companion</p>

            {/* tab switcher */}
            <div style={{display:"flex",gap:0,marginBottom:22,background:"rgba(255,255,255,.05)",borderRadius:14,padding:4}}>
              {[["pin","🔑 PIN Login"],["otp","📱 OTP Login"]].map(([id,lbl])=>(
            ...
            {authTab==="pin" && <>
              ...
              <div style={{marginBottom:15}}>
                ...
                <div style={{display:"flex",gap:9}}>
                  ...
                  <div style={{flex:1,position:"relative"}}>
                    <input value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,""))} placeholder="Enter phone number" type="tel" maxLength={11} style={{...inputStyle(false),paddingRight:50}}/>
              ...
              <div style={{marginBottom:15}}>
                ...
                <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Enter your email" type="email" style={inputStyle(false)}/>
              </div>
              {/* pin */}
              <div style={{marginBottom:10}}>
                <span style={lbl}>PIN / Password</span>
                <div style={{position:"relative"}}>
                  <input value={pin} onChange={e=>setPin(e.target.value.replace(/\D/g,""))} placeholder="4–6 digit PIN" type={showPin?"text":"password"} maxLength={6} style={{...inputStyle(!!authErr),paddingRight:52}} onKeyDown={e=>e.key==="Enter"&&doLogin()}/>
              ...
              <button onClick={()=>showAlert?.("Biometric coming soon!")||notify("👆  Biometric scan initiated…")} style={{...bigBtn("rgba(14,116,144,.18)"),border:"2px solid rgba(14,116,144,.3)",color:"#67e8f9",marginBottom:14}}>
                👆 Fingerprint / Face ID
              </button>
              <div style={{display:"flex",justifyContent:"space-between",paddingTop:4}}>
                <button onClick={()=>notify("Reset link sent to your email!")} style={{background:"none",border:"none",color:"#60a5fa",fontSize:13,cursor:"pointer",fontWeight:700,fontFamily:F}}>Forgot PIN?</button>
                <button onClick={()=>notify("Registration coming soon!")} style={{background:"none",border:"none",color:"#60a5fa",fontSize:13,cursor:"pointer",fontWeight:700,fontFamily:F}}>New User? Register</button>
              </div>
            </>}
          ...
          <div style={{margin:"18px 18px 0",animation:"st_fadeUp .8s ease .2s both"}}>
            <button onClick={triggerPanic} style={{...bigBtn(panicOn?"rgba(127,29,29,.6)":"linear-gradient(135deg,#dc2626,#991b1b)","0 0 28px rgba(220,38,38,.5)"),border:"2px solid rgba(239,68,68,.45)",animation:panicOn?"st_glow 1s infinite":"none"}}>
              {panicOn?"📞 Calling Guardian…":"🆘 EMERGENCY — Call Guardian"}
            </button>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:22,margin:"18px 18px 0",padding:"13px 18px",...glass({borderRadius:14})}}>
            {["🔐 Encrypted","🛡️ 2FA Secure","🔒 Zero Trust"].map(b=>(
    ...
    <div style={{padding:"0 0 108px"}}>
      ...
      <div style={{...cardWrap({background:"linear-gradient(135deg,rgba(29,78,216,.12),rgba(14,116,144,.09))",border:"1px solid rgba(29,78,216,.22)"})}}>
        <div style={{padding:"22px",textAlign:"center"}}>
          ...
          <div style={{fontSize:22,fontWeight:900,fontFamily:FD,color:scam?"#f87171":callOn?"#fbbf24":"#60a5fa",marginBottom:4}}>
            {scam?"SCAM DETECTED!":callOn?`Monitoring Call… ${fmt(callSec)}`:"You're Protected"}
          </div>
          <div style={{fontSize:13,color:"#475569"}}>{scam?"Tap FREEZE now!":callOn?"AI is analysing in real-time":"SafeTap is actively monitoring"}</div>
      ...
      {(callOn||scam) && (
        <div style={{background:"linear-gradient(135deg,rgba(239,68,68,.14),rgba(127,29,29,.1))",border:"1px solid rgba(239,68,68,.38)",borderRadius:18,margin:"14px 16px 0",padding:20,boxShadow:"0 0 28px rgba(239,68,68,.1)"}}>
          ...
          {phrases.length>0 && (
            <div>
              <div style={{fontSize:11,color:"#94a3b8",marginBottom:8}}>🎯 Suspicious Phrases Flagged:</div>
      ...
      {gAlert && (
        <div style={{...cardWrap({background:"rgba(20,83,45,.2)",border:"1px solid rgba(22,163,74,.3)"})}}>
          <div style={{padding:16,display:"flex",alignItems:"center",gap:13}}>
            ...
            <div>
              <div style={{fontSize:14,fontWeight:700,color:"#86efac"}}>Guardians Alerted ✅</div>
              <div style={{fontSize:12,color:"#4ade80"}}>{guardians.map(g=>g.name).join(" & ")} · SMS + WhatsApp sent</div>
            </div>
          </div>
        </div>
      )}
      {frozen && (
        <div style={{...cardWrap({background:"rgba(15,23,42,.3)",border:"1px solid rgba(56,189,248,.3)"})}}>
          <div style={{padding:16,display:"flex",alignItems:"center",gap:13}}>
            <div style={{fontSize:30}}>🔒</div>
            <div>
              <div style={{fontSize:14,fontWeight:700,color:"#7dd3fc"}}>Transaction Frozen ✅</div>
      ...
      <div style={{display:"flex",gap:11,padding:"12px 16px 0"}}>
        {!callOn
          ? <button onClick={startCall} style={{...pill("linear-gradient(135deg,#5b21b6,#4c1d95)"),flex:1,padding:"15px 0",fontSize:14}}>📞 Simulate Scam Call</button>
          : <button onClick={endCall}   style={{...pill("rgba(55,65,81,.8)","#e2e8f0",{border:"1px solid rgba(255,255,255,.1)"}),flex:1,padding:"15px 0",fontSize:14}}>📵 End Call</button>
        }
        <button onClick={downloadReport} style={{...pill("linear-gradient(135deg,#065f46,#064e3b)"),flex:1,padding:"15px 0",fontSize:14}}>📄 Download Report</button>
      </div>
      <div style={{margin:"14px 16px 0",padding:"13px 16px",...glass(),borderRadius:14}}>
        <div style={{fontSize:10,color:"#475569",marginBottom:4,fontWeight:700,letterSpacing:.8,textTransform:"uppercase"}}>Application Tracking Number</div>
        <div style={{fontSize:17,fontWeight:900,color:"#60a5fa",letterSpacing:1,fontFamily:FD}}>{APP_NUM}</div>
        <div style={{fontSize:11,color:"#334155",marginTop:2}}>Track at cybercrime.gov.in · Helpline: 1930</div>
      </div>
    </div>
  );
  ...
  const ProfileScreen = () => {
    ...
    return (
      <div style={{padding:"0 0 108px"}}>
        ...
        {editMode && (
          <div style={cardWrap()}>
            <div style={{...cHead}}><span style={{fontSize:20}}>🖼️</span><span style={cTitle}>Choose Avatar</span></div>
        ...
        <div style={cardWrap()}>
          <div style={{...cHead,justifyContent:"space-between"}}>
            <div style={{display:"flex",gap:10,alignItems:"center"}}><span style={{fontSize:20}}>👤</span><span style={cTitle}>Personal Information</span></div>
            {!editMode
              ? <button onClick={()=>{setDraft({...profile});setEditMode(true);}} style={{...pill("linear-gradient(135deg,#1d4ed8,#0369a1)"),fontSize:12,padding:"8px 16px"}}>✏️ Edit Profile</button>
              : <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>setEditMode(false)} style={{...pill("rgba(255,255,255,.08)","#94a3b8"),fontSize:12,padding:"8px 14px"}}>Cancel</button>
                  <button onClick={saveProfile} style={{...pill("linear-gradient(135deg,#065f46,#064e3b)"),fontSize:12,padding:"8px 16px"}}>💾 Save</button>
                </div>
            }
          </div>
          <div style={{padding:"16px 20px"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
              <div><Row k="name" label="Full Name"/></div>
              <div><Row k="age"  label="Age" type="number"/></div>
              <div><Row k="dob"  label="Date of Birth"/></div>
              <div>
            ...
            <Row k="address" label="Home Address"/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0 10px"}}>
              <div><Row k="city"    label="City"/></div>
              <div><Row k="state"   label="State"/></div>
              <div><Row k="pincode" label="Pincode"/></div>
            </div>
          </div>
        </div>
        ...
        {/* security info */}
        <div style={cardWrap()}>
          <div style={{...cHead}}><span style={{fontSize:20}}>🔐</span><span style={cTitle}>Security & Identity</span></div>
          <div style={{padding:"16px 20px"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
              <div><Row k="phone"   label="Phone" type="tel"/></div>
              <div><Row k="email"   label="Email"  type="email"/></div>
              <div><Row k="aadhaar" label="Aadhaar (Last 4)"/></div>
              <div>
                <span style={lbl}>Verification Status</span>
                <div style={{padding:"11px 14px",...glass({borderRadius:12,border:"1px solid rgba(22,163,74,.2)",background:"rgba(22,163,74,.07)"})}}>
                  <div style={{fontSize:12,color:"#4ade80",fontWeight:700}}>📱 Phone: Verified ✅</div>
                  <div style={{fontSize:12,color:"#4ade80",fontWeight:700,marginTop:4}}>📧 Email: Verified ✅</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* bank */}
        <div style={cardWrap()}>
          <div style={{...cHead}}><span style={{fontSize:20}}>🏦</span><span style={cTitle}>Bank Details</span></div>
          <div style={{padding:"16px 20px"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
              <div><Row k="bank" label="Bank Name"/></div>
              <div><Row k="acc"  label="Account (Last 4)"/></div>
            </div>
          </div>
        </div>
        ...
        {/* actions */}
        <div style={{display:"flex",gap:11,padding:"12px 16px 0"}}>
          <button onClick={()=>setLang(l=>l==="en"?"hi":"en")} style={{...pill("rgba(29,78,216,.12)","#93c5fd",{border:"2px solid rgba(29,78,216,.28)"}),flex:1,padding:"14px 0",fontSize:14}}>🌐 {lang==="en"?"हिंदी":"English"}</button>
          <button onClick={()=>setLoggedIn(false)} style={{...pill("rgba(239,68,68,.1)","#fca5a5",{border:"2px solid rgba(239,68,68,.24)"}),flex:1,padding:"14px 0",fontSize:14}}>🚪 Logout</button>
        </div>
      </div>
    );
  };
  ...
  const GuardiansScreen = () => (
    <div style={{padding:"0 0 108px"}}>
      <div style={cardWrap()}>
        <div style={{...cHead,justifyContent:"space-between"}}>
          <div style={{display:"flex",gap:10,alignItems:"center"}}><span style={{fontSize:22}}>👨‍👩‍👧‍👦</span><span style={cTitle}>My Guardians</span></div>
          <button onClick={()=>setAddGOpen(true)} style={{...pill("linear-gradient(135deg,#1d4ed8,#0369a1)"),fontSize:13,padding:"9px 16px"}}>+ Add Guardian</button>
        </div>
        <div style={{padding:"14px 20px"}}>
          {guardians.length===0 && <div style={{textAlign:"center",padding:"24px 0",color:"#475569",fontSize:14}}>No guardians yet. Add a trusted family member.</div>}
          {guardians.map((g,i)=>(
            <div key={g.id} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 0",borderBottom:i<guardians.length-1?"1px solid rgba(255,255,255,.06)":"none"}}>
              ...
              <button onClick={()=>{setGuardians(p=>p.filter(x=>x.id!==g.id));notify("Guardian removed","warn");}} style={{...pill("rgba(239,68,68,.14)","#fca5a5",{border:"1px solid rgba(239,68,68,.3)"}),padding:"8px 12px",fontSize:12}}>Remove</button>
            </div>
          ))}
        </div>
      </div>
      ...
      {addGOpen && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setAddGOpen(false)}>
          <div style={{width:"100%",maxWidth:480,...glass({borderRadius:"24px 24px 0 0",border:"1px solid rgba(29,78,216,.25)"}),padding:"26px 22px 44px",animation:"st_pop .3s ease"}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:20,fontWeight:900,color:"#f8fafc",fontFamily:FD,marginBottom:20}}>+ Add Guardian</div>
            {[{k:"name",l:"Full Name",t:"text",ph:"e.g. Priya Kumar"},{k:"phone",l:"Phone Number",t:"tel",ph:"+91-98765-43210"}].map(f=>(
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:22}}>
              {RELS.map(r=>(
                <button key={r} onClick={()=>setNewG(p=>({...p,relationship:r}))} style={{...pill(newG.relationship===r?"linear-gradient(135deg,#1d4ed8,#0369a1)":"rgba(255,255,255,.07)",newG.relationship===r?"#fff":"#64748b",{border:newG.relationship===r?"none":"1px solid rgba(255,255,255,.1)"}),padding:"9px 14px",fontSize:13}}>
                  {r}
                </button>
              ))}
            </div>
            <div style={{display:"flex",gap:11}}>
              <button onClick={()=>setAddGOpen(false)} style={{...pill("rgba(255,255,255,.07)","#94a3b8",{border:"1px solid rgba(255,255,255,.1)"}),flex:1,padding:"16px 0",fontSize:15}}>Cancel</button>
              <button onClick={addGuardian} style={{...pill("linear-gradient(135deg,#1d4ed8,#0e7490)"),flex:2,padding:"16px 0",fontSize:16,fontFamily:FD}}>Save Guardian</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{...cardWrap({background:"rgba(20,83,45,.14)",border:"1px solid rgba(22,163,74,.2)"})}}>
        <div style={{padding:16}}>
          <div style={{fontSize:13,color:"#86efac",lineHeight:1.8}}>✅ All guardians instantly receive <strong style={{color:"#4ade80"}}>SMS + WhatsApp alerts</strong> when SafeTap detects a scam or you press FREEZE TRANSACTION.</div>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{padding:"0 0 108px"}}>
      <div style={cardWrap()}>
        <div style={{...cHead}}><span style={{fontSize:22}}>⚖️</span>
          <div><div style={cTitle}>Complaint Progress</div><div style={{fontSize:11,color:"#475569"}}>App No: {APP_NUM}</div></div>
        </div>
        <div style={{padding:"22px",display:"flex",flexDirection:"column",alignItems:"center"}}>
          ...
          <div style={{marginTop:11,fontSize:13,color:"#64748b",textAlign:"center"}}>
            {progress<100?"Complaint is actively being processed by authorities":"Case Resolved ✅"}
          </div>
        </div>
      </div>

      <div style={cardWrap()}>
        <div style={{...cHead}}><span style={{fontSize:22}}>📋</span><span style={cTitle}>Step-by-Step Tracker</span></div>
      ...
      <div style={{...cardWrap({background:"rgba(6,78,59,.14)",border:"1px solid rgba(16,185,129,.22)"})}}>
        <div style={{...cHead}}><span style={{fontSize:22}}>📄</span><span style={cTitle}>Evidence Report</span></div>
        <div style={{padding:"14px 20px"}}>
          ...
          <p style={{margin:"10px 0 0",fontSize:12,color:"#334155",textAlign:"center"}}>Includes FIR, caller transcript, bank freeze & all complaint references</p>
        </div>
      </div>
    </div>
  );
  ...
  const InfoScreen = () => (
    <div style={{padding:"0 0 108px"}}>
      <div style={cardWrap()}>
        <div style={{...cHead}}><span style={{fontSize:22}}>ℹ️</span><span style={cTitle}>About SafeTap</span></div>
        <div style={{padding:"14px 20px"}}>
          {[
            {icon:"🤖",t:"AI Scam Detection",d:"Real-time NLP detects 50+ scam patterns, pressure cues and coercion in live calls"},
            {icon:"🔒",t:"Freeze Transaction",d:"One tap instantly blocks any pending bank transfer"},
            {icon:"👨‍👩‍👧‍👦",t:"Guardian Alerts",d:"Instant SMS + WhatsApp to all registered guardians"},
            {icon:"⚖️",t:"Auto Complaint Filing",d:"Auto-files at cybercrime.gov.in and nearest police station"},
            {icon:"📄",t:"Full Evidence Report",d:"Downloadable report with FIR, caller evidence, bank details & tracking"},
          ].map(f=>(
      ...
      <div style={{...cardWrap({background:"rgba(69,10,10,.2)",border:"1px solid rgba(239,68,68,.2)"})}}>
        <div style={{padding:18}}>
          <div style={{fontSize:17,fontWeight:900,fontFamily:FD,color:"#f87171",marginBottom:14,textAlign:"center"}}>🆘 Emergency Numbers</div>
          {[["Cyber Crime","1930"],["Police","100"],["SafeTap","1800-SAFE-TAP"]].map(([k,v])=>(
